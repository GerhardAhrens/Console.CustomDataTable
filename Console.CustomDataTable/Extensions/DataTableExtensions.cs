...
namespace Console.CustomDataTable
{
    ...
    public static class DataTableExtensions
    {
        ...
        public static string DataTableToCode(this DataTable Table)
        {
            ...
            {
                codeClass.Members.Add(CreateProperty(column.ColumnName, column.DataType,column.AllowDBNull));
            }
            ...
            codeClass.Members.Add(CreateMethode("ToString"));
        ...
        private static CodeTypeDeclaration CreateClass(string name)
        {
            ...
            result.BaseTypes.Add(typeof(DataRow)); // Add Vererbung
        ...
        private static CodeConstructor CreateConstructor(string className)
        {
            ...
            result.Parameters.Add(new CodeParameterDeclarationExpression(typeof(DataRowBuilder), "builder"));
            result.BaseConstructorArgs.Add(new CodeVariableReferenceExpression("builder"));

            CodeFieldReferenceExpression fieldRef = new CodeFieldReferenceExpression(new CodeThisReferenceExpression(), "Id");
            CodeArgumentReferenceExpression argRef = new CodeArgumentReferenceExpression("Guid.NewGuid()");
            result.Statements.Add(new CodeAssignStatement(fieldRef, argRef));

            return result;
        }

        private static CodeMemberField CreateProperty(string name, Type type, bool isNullable)
        {
        ...
        private static CodeMemberMethod CreateMethode(string methodeName)

        {
            CodeMemberMethod resultMethod = new CodeMemberMethod();
            resultMethod.Name = methodeName;
            resultMethod.ReturnType = new CodeTypeReference(typeof(string));
            resultMethod.Attributes = MemberAttributes.Public | MemberAttributes.Override; // WICHTIG: Überschreiben
